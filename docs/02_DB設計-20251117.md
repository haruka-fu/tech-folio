...
| user_id | TEXT | NOT NULL, UNIQUE, 4-20文字 | ユーザーID（初回登録時+1回変更可能） |
| user_id_change_count | INTEGER | DEFAULT 0, 最大1 | user_id変更回数（最大1回まで） |
...
- user_idは初回登録時と1回のみ変更可能（合計2回まで使用）
- SNS URLは全て手動入力
...
1. **user_id変更制限**: 初回登録時+1回のみ変更可能（合計2回まで）
